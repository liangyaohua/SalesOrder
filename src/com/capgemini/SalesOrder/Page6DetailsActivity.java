 ...
 * Details screen.
 */
public class Page6DetailsActivity extends ListActivity implements IZGWSAMPLE_SRVRequestHandlerListener 
{
	public static final String TAG = "SalesOrder";
	private ISDMLogger logger;
	
	protected static Product parentEntry;
	...
	private Page6DetailsAdapter adapter; 
	...
	protected void updateResultsInUi()
	{
		...
		adapter = new Page6DetailsAdapter(this, entry);
	...
	protected void onListItemClick(ListView l, View v, int position, long id) 
	{
		...
		Page6DetailsAdapter.SapSemantics sapSemantics = adapter.getSapSemantics(position);
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		ZGWSAMPLE_SRVRequestHandler.getInstance(getApplicationContext()).register(this, ZGWSAMPLE_SRVRequestID.LOAD_SUPPLIER_FOR_PRODUCT);
		
		// make the request
		// the response should be in "requestCompleted"
		ZGWSAMPLE_SRVRequestHandler.getInstance(getApplicationContext()).loadSupplierForProduct(parentEntry);
	}
	...
	public void requestCompleted(ZGWSAMPLE_SRVRequestID requestID, List<?> entries, RequestStatus requestStatus) 
	{
		...
		{
			if (requestID.equals(ZGWSAMPLE_SRVRequestID.LOAD_SUPPLIER_FOR_PRODUCT))
			{
				// cast to the right type
				this.entry = (BusinessPartner) entries.get(0);
				// post in the UI
